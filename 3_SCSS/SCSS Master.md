...
지금까지는 부모(flexbox container)가 어떻게 자식을 옮기는가에 대해 설명했다면, 이제는 자식 스스가 자신을 설정하는 2가지 옵션에 대해 배울 것이다. 부모에 설정하면 모든 자식 요소 전부에 적용되지만, 특정 자식 요소에만 적용하고 싶은 경우 사용한다.
...
## 3.3 Extends
Mixin이 상황에 따라 다르게 적용되도록 코딩하고 싶을 때 사용했다면, Extend는 같은 코드를 중복하고 싶지 않을 때 or 다른 코드를 확장하고 싶을 때 사용한다.
